...
async def async_setup_entry(
    ...
    entities = [
        ...
        # Kiszellőztetés / Szabadhűtés időtartamok
...
class SmartClimateVentNumber(NumberEntity):
    ...
    def __init__(self, coordinator, config_entry, param_type, name, min_val, max_val, unit, step=1, mode=None):
        ...
        if param_type == "humidity":
             ...
             self._attr_mode = NumberMode.SLIDER
        elif param_type in ["airout_duration", "free_cooling_max_duration"]:
             self._attr_icon = "mdi:timer-sand"
        elif param_type == "free_cooling_cooldown":
             self._attr_icon = "mdi:snowflake-clock"
        else:
    ...
    def native_value(self):
        ...
        elif self._param_type == "free_cooling_max_duration":
            return self.coordinator.free_cooling_max_duration
        elif self._param_type == "free_cooling_cooldown":
            return self.coordinator.free_cooling_cooldown
        return 0
    ...
    async def async_set_native_value(self, value: float) -> None:
        if self._param_type == "humidity":
            ...
            self.coordinator.free_cooling_max_duration = value
        elif self._param_type == "free_cooling_cooldown":
            self.coordinator.free_cooling_cooldown = value
        elif self._param_type == "fan_speed":
